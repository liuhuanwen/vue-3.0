  ...
  <div class="container">
    <div class="header" @click="handleClickHeader()">
      ...
      <div class="profile-wrapper">
        <p class="username">
          {{ user.username ? user.username : "登录/注册" }}
        </p>
        <p class="mobile">
          <svg>
          ...
          <span
            >&nbsp;{{
              encryptionMobile ? encryptionMobile : "登录后享受更多特权"
            }}</span
          >
        </p>
      </div>
    ...
    <div class="body">
      <section>
        <a class="red-packet-wrapper">
          ...
          <svg
            v-if="user.gift_amount === undefined"
            fill="#ff5f3e"
            class="icon-red-packet"
          >
  ...
  computed: {
    ...
    encryptionMobile() {
      if (this.user && this.user.mobile) {
        return (
          this.user.mobile.substring(0, 3) +
          "****" +
          this.user.mobile.substring(7, 11)
        );
      }
      return "";
    },
